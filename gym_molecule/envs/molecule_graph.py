...
import torch
from torchdrug.metrics.rdkit.sascorer import calculateScore
...
def get_final_mol(mol):
    # m = Chem.ReplaceSubstructs(Chem.MolFromSmiles(smi), Chem.MolFromSmiles("*"))
    m = Chem.ReplaceSubstructs(mol, Chem.MolFromSmiles('*'), Chem.MolFromSmiles('[H]'),
                               replaceAll=True)
    Chem.SanitizeMol(m[0], sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
    return m[0]
...
def reward_vina(smis, predictor, reward_vina_min=0):
    ...
    reward = np.clip(reward, reward_vina_min, None)  # 正常来说其应当都为正值，若为负值，则最小为0
    return reward
...
class MoleculeEnv(gym.Env):
    ...
    def init(self, docking_config=dict(), data_type='zinc', ratios=dict(), reward_step_total=1, is_normalize=0,
        ...
        self.max_atom = 150
        self.action_space = gym.spaces.MultiDiscrete([len(FRAG_VOCAB), 40])
    ...
    def reward_batch(self):
        ...
        reward_batch_vina = reward_vina(self.smile_list, self.predictor)
        return reward_vina(self.smile_list, self.predictor)

    def reward_single(self, smile_list):
        print('smiles list', smile_list)
        # return reward_vina(smile_list, self.predictor) - calculateScore(Chem.MolFromSmiles(smile_list[0]))#因为此处已经将
        # return reward_vina(smile_list, self.predictor), calculateScore(Chem.MolFromSmiles(smile_list[0]))  # 因为此处已经将
        # return calculateScore(Chem.MolFromSmiles(smile_list[0]))
        return reward_vina(smile_list, self.predictor)

    def step(self, ac, final_smiles_list, bond_idx_list):
        """
        ...
        if (self.counter > self.max_action) or get_att_points(self.mol) == []:
            ...
            self.mol = Chem.MolFromSmiles(final_smiles_list[0][ac[1]])
            self.bond_idx = bond_idx_list[0][ac[1]]
            # info['smile'] = get_final_smi(Chem.MolToSmiles(self.mol))
        reward = 0
        reward_step = 0.05
        # reward_step = 0
        # sa_diff = calculateScore(get_final_mol(self.mol_old))-calculateScore(get_final_mol(self.mol))
        ...
        if new:
            ...
            if get_att_points(self.mol) != []:
                ...
                try:
                    Chem.SanitizeMol(mol_old_no_att[0], sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
                except:
                    pass
    ...
    def reset(self, smile=None):
        ...
        self.counter = 1
        ob = self.get_observation()  # 需要修改为使用pyg来求解分子特征
        return ob
    ...
    def _add_motif(self, ac):
        ...
        motif = Chem.ReplaceSubstructs(motif, self.K)
    ...
    def get_final_mol(self):
        ...
        """
        # m = Chem.DeleteSubstructs(self.mol, Chem.MolFromSmiles("*"))
        m = Chem.ReplaceSubstructs(self.mol, Chem.MolFromSmiles('*'), Chem.MolFromSmiles('[H]'), replaceAll=True)
        return m[0]

    def get_final_mol_ob(self, mol):
        # m = Chem.DeleteSubstructs(mol, Chem.MolFromSmiles("*"))
        m = Chem.ReplaceSubstructs(mol, Chem.MolFromSmiles('*'), Chem.MolFromSmiles('[H]'), replaceAll=True)
        return m
